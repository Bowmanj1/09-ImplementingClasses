...
class Point(object):
    ...
    def get_distance_from(self, point):
        dist = ((self.x - point.x) ** 2 + (self.y - point.y) ** 2) ** (1 / 2)
        return dist
    ...
    def move_to(self, x, y):
        ...
        self.dist_t = self.dist_t + ((self.x - x2) ** 2 + (self.y - y2) ** 2) ** (1 / 2)
    ...
    def move_by(self, dx, dy):
        ...
        self.x = self.x + dx
        self.y = self.y + dy
        self.dist_t = self.dist_t + ((self.x - x1) ** 2 + (self.y - y1) ** 2) ** (1 / 2)
        self.count = self.count + 1
    ...
    def get_distance_from_start(self):
        dist2 = ((self.x - self.startx) ** 2 + (self.y - self.starty) ** 2) ** (1/2)
        return dist2
    ...
    def closer_to(self, p2, p3):
        dist2 = ((self.x - p2.x) ** 2 + (self.y - p2.y) ** 2) ** (1 / 2)
        dist3 = ((self.x - p3.x) ** 2 + (self.y - p3.y) ** 2) ** (1 / 2)
        if dist2 > dist3:
            return p3
        else:
            return p2
